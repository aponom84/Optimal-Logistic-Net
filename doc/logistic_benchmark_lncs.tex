...
\author{Varvara Kovaleva \and Alexander Ponomarenko}
\authorrunning{V.\,Kovaleva and A.\,Ponomarenko}

\institute{HSE University, Laboratory of Algorithms and Technologies for Network
Analysis, Nizhny Novgorod, Russia\\
\email{aponom84@gmail.com}}
...
model. We release 34 fully synthetic instances ($10$--$150$ storages) and 15
...
orders of magnitude. The benchmark is a faithful, reusable testbed for exact and
heuristic solvers.

\keywords{Benchmark generation \and Synthetic data \and Spatial networks \and
Multi-commodity flow \and Gravity model \and Zipf's law \and Reproducibility.}
\end{abstract}
...
public benchmarks. Classical multi-commodity flow generators and libraries do
...
make solver comparisons meaningful and reproducible.
...
are released under permissive licences (data under CC~BY~4.0, code under MIT) and
archived with a versioned DOI following FAIR principles \cite{fair}; an
interactive web viewer (Streamlit/Plotly) ships alongside for inspecting
instances and solver outputs.
...
A benchmark is only useful if its instances vary in difficulty and separate
solvers. We illustrate this with three reference quantities obtained on the
corpus: the optimum of the linear relaxation (LP, a lower bound), the relaxation
rounded up to integer vehicle counts (Rounded LP, a feasible upper bound), and an
exact MILP solved by HiGHS \cite{highs} under a wall-clock budget proportional to
size. To place them on a common scale we report each as a ratio to the objective
returned by a metaheuristic solver (GVNS) run within the same budget; a value
below $1$ is a lower bound and above $1$ is worse.

Table~\ref{tab:usecase} and Figure~\ref{fig:ratios} show two clear trends.
First, the LP lower bound is tight on small instances (ratio $0.97$ at $|V|=10$)
and loosens steadily with size (down to $0.36$ at $|V|=150$): the relaxation gap,
a proxy for combinatorial hardness, grows by roughly an order of magnitude across
the corpus. Second, the exact MILP becomes impractical exactly where the gap
widens---for $|V|\ge100$ it returns no solution within the budget---while the
Rounded-LP upper bound degrades from $1.19\times$ to over $2\times$. The corpus
therefore spans the full range from instances solvable to near-optimality to
instances on which exact methods fail in reasonable time, which is precisely the
discrimination a benchmark must provide. The real-geometry instances
(Fig.~\ref{fig:ratios}, right) reproduce these trends quantitatively: the LP
ratio falls from $0.99$ on Sweden ($|V|=45$) to $0.76$ on European Russia
($|V|=90$), while the exact MILP---feasible only up to $|V|=59$ within the
budget---already costs $1.3$ to $1.5\times$ the metaheuristic objective on the
larger U.S.\ states (e.g.\ $1.48$ on Illinois, $1.38$ on Georgia). That the same
hardness signature appears on networks built from real geography confirms that
difficulty, and not only structure, transfers between the synthetic and
real-geometry sets---so a solver tuned on the synthetic corpus can be expected to
behave comparably on real inputs.

\begin{table}[t]
\centering
\caption{Reference quantities on the synthetic set, aggregated by size and
expressed as ratios to the metaheuristic (GVNS) objective. ``--'' marks sizes
where the exact MILP returned no solution within the time budget.}
\label{tab:usecase}
\setlength{\tabcolsep}{8pt}
\begin{tabular}{rrrrr}
\toprule
$|V|$ & $n$ & LP (lower) & Rounded LP (upper) & Exact MILP\\
\midrule
10  & 10 & 0.970 & 1.191 & 1.001\\
20  & 7  & 0.962 & 1.577 & 1.014\\
30  & 7  & 0.916 & 2.177 & 1.071\\
50  & 5  & 0.802 & 2.776 & 1.543\\
100 & 3  & 0.524 & 2.114 & --\\
150 & 3  & 0.359 & 2.002 & --\\
\bottomrule
\end{tabular}
...
\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{lfigs/ratios.png}
\caption{Reference quantities as ratios to GVNS versus network size. Left:
synthetic instances (means per size). Right: real-geometry networks. The
LP lower bound diverges and the exact MILP becomes infeasible as size grows,
demonstrating that the corpus discriminates difficulty.}
\label{fig:ratios}
\end{figure}
...
files shows that the $34$ synthetic and $15$ real-geometry instances are
...
generator and viewer are released together so that solver comparisons on this
problem can be reproducible and externally valid.

\bigskip
\noindent\textbf{Data availability.} The generator, the instance corpus and the
viewer are released openly (data: CC~BY~4.0; code: MIT) and archived with a
versioned DOI; the development repository is hosted on GitHub.
